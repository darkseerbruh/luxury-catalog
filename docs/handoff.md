...
| **Data / capture** | `…/luxury-catalog-data` · `data/market-capture` | `supabase/ingest/**` · `scripts/**` · `data/ingest/**` | [data-collection-handoff.md](data-collection-handoff.md) + [capture-runbook.md](capture-runbook.md) + [market-sweep-worklist.md](market-sweep-worklist.md) | **2026-06-26: catalog jumped 401→750 priced variants, 19.2k→23.1k asking rows + the first 421 peer-to-peer SOLD rows (Coach Tabby/Rogue, eBay).** `promote-safe.ts` rolled 3,855 discovered asking rows into 85 new curated size-variants of EXISTING styles (Triomphe, Horsebit 1955, Vanity Case, Evelyne, Lady Dior, Constance, Keepall…); `audit-coverage.ts` is the gap auditor. **Transport reality (browser→repo): use `get_page_text` body-transport** — the localhost sink is CSP-blocked on eBay/Poshmark/TRR, and Chrome gates all but the FIRST blob-download per origin. **Next:** (1) **28 promote-safe clusters need NEW styles → owner greenlight** (data-collection-handoff §12); (2) mid-tier breadth (MK/Kate Spade/Longchamp via eBay/Poshmark + the **Redeluxe + Couture USA open Shopify feeds**, see `market-sweep-worklist.md` "Incoming to-dos" + `docs/trusted-resellers.md`); (3) hero p2p SOLD (Chanel/LV/Hermès) to back value articles. **(4) Content-chat ask: run the Google Trends pull `docs/research-drafts/trends-keyword-pull.md` while you're in the browser (7 compare URLs, record directional rankings in its FINDINGS section) so Content can prioritize articles to real demand.** |
...
- **Hero p2p SOLD started:** +78 Chanel Classic Flap Medium realized prices loaded to v199 (eBay, median $3,846, corroborates the prior $3,897 read). So p2p sold now exists for both Coach and the Flap. Still queued: LV/Hermès heroes, mid-tier breadth (MK/Kate Spade/Longchamp/Mulberry — absent), Poshmark cross-source, the 28 new-style promotions.
