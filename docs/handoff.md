...
| **Seasonal archive / intelligence** (`archivist` subagent) | any clean worktree · land doc-only to `main` via a guarded worktree | `docs/research-drafts/seasonal-archive/**` · `docs/seasonal-archive-worklist.md` · `.claude/agents/archivist.md` · `.mcp.json` | [seasonal-archive-worklist.md](seasonal-archive-worklist.md) | **2026-06-28 (large autonomous run, all on `main`):** built the `archivist` subagent (handbag intelligence + 30-year seasonal naming archive) and landed an **11-house lexicon** (Hermès/Chanel/LV/Dior/Gucci/Bottega/YSL/Celine/Fendi/Prada/Loewe = ~550 sourced name rows; Hermès now carries 2013-2025 seasonal colors + staple color codes). Key finding: **three color-naming camps** (Hermès+Bottega name their colors; Dior/Gucci/YSL/Celine/Fendi/Prada do not; Loewe in between); **Chanel season codes ARE names** (decoder article + an interactive decoder TOOL idea). **6 GEO article DRAFTS** in `docs/research-drafts/seasonal-archive/drafts/` (Chanel decoder, Hermès color-codes + leathers, which-houses-name-colors, Prada Saffiano-vs-nylon, Bottega names) + a **25-idea content slate** (`content-ideas.md`, GEO/engagement tagged). **Free-tier data paths wired:** Firecrawl (open web + discovery) + Chrome extension (login-walled, owner present) + **Apify connected via `.mcp.json`** (unattended TikTok/IG, capped, $5/mo free). **Next (open):** (1) owner-present **Chrome `.com` batch** for the deep recent color years of LV/Dior/Gucci/BV/Celine/YSL/Fendi/Prada/Loewe (Akamai-blocks Firecrawl); (2) hand more slate ideas to the `copywriter`; (3) the **decoder TOOL is a UX-lane build**; (4) **Content lane seeds the drafts as live posts** (owner-gated). **Caution:** env auto-switches the working branch (saw `docs/analytics-session-wrapup`), so land via a guarded worktree off `origin/main` and check shared-file (worklist/slate) regressions before landing. |
