...
|---|---|---|---|---|
| **Content** (editorial suite) | `…/luxury-catalog-content` · `content/editorial` | `post` rows · `src/app/posts/**` · `docs/content-*.md` | [content-writing-handoff.md](content-writing-handoff.md) + [content-strategy.md](content-strategy.md) | Coach authentication guide SEEDED as a draft (`post_id 2`, slug `how-to-authenticate-a-coach-bag`, topic Coach), built with an original schematic diagram component (`CoachAuthDiagram.tsx`, desktop ring + mobile per-tile stack) rendered via a `[diagram:]` body token. Awaiting owner review, then owner publishes. Chanel draft `post_id 1` still failed its audit. 0 published. **Next:** owner publishes Coach → write the rest of the suite (comparisons + where-to-sell). |
