...
|---|---|---|---|---|
| **Content** (editorial suite) | `…/luxury-catalog-content` · `content/editorial` | `post` rows · `src/app/posts/**` · `docs/content-*.md` | [content-writing-handoff.md](content-writing-handoff.md) + [content-strategy.md](content-strategy.md) | Coach authentication guide **PUBLISHED** (`post_id 2`, `/posts/how-to-authenticate-a-coach-bag`, topic Coach), by **Arielle** (verified account, author card + bio). Built with an original schematic diagram component (`CoachAuthDiagram.tsx`: 6-tile grid + per-section visuals via `[diagram:]` body tokens). **1 published.** Chanel draft `post_id 1` still failed its audit. **Next:** write the rest of the suite (a comparison + the where-to-sell guide), then re-apply to Skimlinks once a few pieces are live. |
