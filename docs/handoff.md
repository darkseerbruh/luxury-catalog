...
| **Infra / ops** (catch-all) | any clean worktree · `ops/<task>` | `supabase/migrations/**` (announce the number!) · deploy/DNS · `docs/desktop-todo.md` · analytics | [desktop-todo.md](desktop-todo.md) + the setup checklist below | Migration apply-state = **RE-VERIFY** (see the pending-operator block below, do not trust). **Analytics layer built 2026-06-28 (5 branches, NOT merged — see this date's TL;DR):** pulse + dashboards, persona-model (G1, needs migration `0035`), bag-compare (G2), journey instrumentation, premium fake-door. **Owner: apply `0035` + merge in order 1,2,4,5.** **Next (open to-do):** site-load / perf investigation, `desktop-todo.md` §J. |
...
**Five branches pushed to origin, NONE merged (owner lands them; `origin/main` advanced + is checked out in the analyst worktree, and G1 carries a migration):**
...
Gates per branch: tsc, **eslint src** (the AGENTS gate; whole-repo `npm run lint` has pre-existing `any` errors in `supabase/ingest/*` from the data lane, not these branches), next build, npm test. **Still open (deliberate):** `inquiry_submitted` has no UI home (needs a lead form — a product decision); `style_viewed` has no style route (retire or repurpose). **Landing order: apply `0035`, then merge 1, 2, 4 (brings 3), 5.**
