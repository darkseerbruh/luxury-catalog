...
- ⬜ [dh:freshness-fashionphile] Fashionphile data is 1.6 days old but its schedule promises a refresh every 0.125 day(s), so the capture likely stopped running. Action: Open GitHub Actions and check the Fashionphile refresh workflow's recent runs. (first seen 2026-07-11, last seen 2026-07-11, yellow)
- ⬜ [dh:ranking-snapshot] No LC Index snapshot exists yet, so the ranking movement pills have nothing to compare against. Action: The monthly cron fires on the 1st (CRON_SECRET verified set in Vercel prod, 2026-07-10); to backfill now, curl /api/cron/lc-index-snapshot with the Bearer secret. (first seen 2026-07-10, last seen 2026-07-11, yellow)
- ⬜ [dh:coverage-hardware] Hardware-color coverage (all rows) fell from 46.8% to 41.1% since the last run. Action: Check the spec-extract vocab (src/lib/ingest/spec-extract.ts) against recent titles. (first seen 2026-07-11, last seen 2026-07-11, red)
- ⬜ [dh:coverage-listing-images] Listing photos (The Luxury Closet) fell from 40.1% to 26.4% since the last run. Action: Check the feed ingest's image upsert (listing_image) in the TLC action. (first seen 2026-07-11, last seen 2026-07-11, red)
- ⬜ [dh:contamination-trr] 7 TRR rows captured in the last week would fail today's non-bag filter, so non-bag items are leaking back in. Action: Run the TRR non-bag cleanup scripts (cleanup-trr-nonbag.ts) and check the ingest gate. (first seen 2026-07-10, last seen 2026-07-11, yellow)
- ⬜ [dh:contamination-dupes] 3338 listings were written more than once for the same observation day, which contaminates medians until deduped. Action: Inspect the newest loads; the loader should upsert on platform|listing_ref|price_type|observed_on. (first seen 2026-07-10, last seen 2026-07-11, red)
<!-- data-health:end -->
