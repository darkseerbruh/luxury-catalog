...
- Speedy Bandoulière construction toggle (seed-lv-strap-variants.ts).
- Alma → Size×Material×Colour; Birkin → leather Material axis (seed-hermes-leather-variants.ts).
- Neverfull PM/GM canvas split (split-canvas-tagged-bases.ts) — full Mono/Ebene/Azur across PM/MM/GM.
- Material dedup (merge-duplicate-materials.ts: Togo/Lambskin) + FIXED dedup-variant-rows.ts key to include construction/hardware/strap (it would have collapsed the Bandoulière axis).
- Templated 4 more via archivist matrices (docs/research-drafts/onthego-ladydior-saintlouis-jackie-production-matrix.md): OnTheGo (Size×Colour), Lady Dior (Size×Colour×Material, 86v), Goyard Saint Louis (11-colour palette), GG Marmont (Size×Colour×Material, 41v). Migrations 0054 already applied.
Parked follow-ons (bounded):
- ⬜ OnTheGo (437) MATERIAL axis: the Giant lines (Monogram Giant / Empreinte / Reverse) need custom title detection; currently Size×Colour only (colour split mixes canvas-brown with Empreinte colours — a material-primary refactor would be cleaner). LV's primary axis, so highest-value OnTheGo fix.
...
- ⬜ Template the remaining ≥500-listing icons (all unfaceted): Celine Luggage (823), Valentino Rockstud (780), YSL Kate (746), BV Jodie (656), Loewe Puzzle (644), Gucci Bamboo 1947 (638), Balenciaga City (587), Prada Re-Edition 2005 (527), Givenchy Antigona (516) = 1 archivist pull + seed each. Then the <500 long tail.
- ⬜ Multi-word MATERIAL sub-axis follow-on: production material names like "Oblique Embroidery"/"Smooth Calf"/"Matelasse Lambskin"/"GG Supreme Canvas"/"Shiny Calfskin" don't substring-match listing text, so those material axes are thin/absent (colour axis is solid). Needs a keyword-based material detector (like seed-lv-line-variants does for LV lines).
...
- ⬜ [dh:freshness-fashionphile] Fashionphile data is 2.6 days old but its schedule promises a refresh every 0.125 day(s), so the capture likely stopped running. Action: Open GitHub Actions and check the Fashionphile refresh workflow's recent runs. (first seen 2026-07-11, last seen 2026-07-12, red)
- ⬜ [dh:ranking-count] The number of ranked styles fell from 331 to 330; a data problem may be knocking styles below the ranking floor. Action: Compare style_index_signals output with the last report to see which styles dropped and why. (first seen 2026-07-12, last seen 2026-07-12, yellow)
- ⬜ [dh:ranking-snapshot] No LC Index snapshot exists yet, so the ranking movement pills have nothing to compare against. Action: The monthly cron fires on the 1st (CRON_SECRET verified set in Vercel prod, 2026-07-10); to backfill now, curl /api/cron/lc-index-snapshot with the Bearer secret. (first seen 2026-07-10, last seen 2026-07-12, yellow)
- ⬜ [dh:coverage-hardware] Hardware-color coverage (all rows) fell from 41.1% to 38.4% since the last run. Action: Check the spec-extract vocab (src/lib/ingest/spec-extract.ts) against recent titles. (first seen 2026-07-11, last seen 2026-07-12, yellow)
- ⬜ [dh:coverage-listing-images] Listing photos (The Luxury Closet) fell from 26.4% to 21.3% since the last run. Action: Check the feed ingest's image upsert (listing_image) in the TLC action. (first seen 2026-07-11, last seen 2026-07-12, red)
- ⬜ [dh:backlog-total] The unpromoted backlog grew from 44854 to 56895 rows since the last run; capture is outpacing promotion. Action: Schedule a promotion session (promote-safe --create-new) or grow the dictionary coverage. (first seen 2026-07-12, last seen 2026-07-12, red)
- ⬜ [dh:contamination-trr] 7 TRR rows captured in the last week would fail today's non-bag filter, so non-bag items are leaking back in. Action: Run the TRR non-bag cleanup scripts (cleanup-trr-nonbag.ts) and check the ingest gate. (first seen 2026-07-10, last seen 2026-07-12, yellow)
- ⬜ [dh:contamination-misattach] 47 accessories (card holders, pouches, shoes) landed on a real bag's variant in the last week, which is exactly what put wrong items on the deals rail; 169 more look like a different-model bag and want a manual review. Action: Run supabase/ingest/detect-listing-discrepancies.ts (dry run to review, --write to re-point the accessories, --tag to label), and check why the ingest match let them onto a bag variant. (first seen 2026-07-12, last seen 2026-07-12, red)
- ⬜ [dh:contamination-size] 816 listings captured in the last week state a size that differs from the variant they landed on (e.g. a mini on the Medium page), which unpopulates the size selector and skews same-size pricing. Action: Run supabase/ingest/resize-variants.ts (dry run, then --write) to re-point them, and check the load-prices size guard. (first seen 2026-07-12, last seen 2026-07-12, red)
- ⬜ [dh:contamination-dupes] 4070 listings were written more than once for the same observation day, which contaminates medians until deduped. Action: Inspect the newest loads; the loader should upsert on platform|listing_ref|price_type|observed_on. (first seen 2026-07-10, last seen 2026-07-12, red)
<!-- data-health:end -->
