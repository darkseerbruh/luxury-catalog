...
- ⬜ [dh:freshness-fashionphile] Fashionphile data is 4.9 days old but its schedule promises a refresh every 0.125 day(s), so the capture likely stopped running. Action: Open GitHub Actions and check the Fashionphile refresh workflow's recent runs. (first seen 2026-07-11, last seen 2026-07-14, red)
- ⬜ [dh:ranking-snapshot] No LC Index snapshot exists yet, so the ranking movement pills have nothing to compare against. Action: The monthly cron fires on the 1st (CRON_SECRET verified set in Vercel prod, 2026-07-10); to backfill now, curl /api/cron/lc-index-snapshot with the Bearer secret. (first seen 2026-07-10, last seen 2026-07-14, yellow)
- ⬜ [dh:coverage-hardware] Hardware-color coverage (all rows) fell from 36% to 33.7% since the last run. Action: Check the spec-extract vocab (src/lib/ingest/spec-extract.ts) against recent titles. (first seen 2026-07-11, last seen 2026-07-14, yellow)
- ⬜ [dh:coverage-listing-images] Listing photos (The Luxury Closet) fell from 18.1% to 15.7% since the last run. Action: Check the feed ingest's image upsert (listing_image) in the TLC action. (first seen 2026-07-11, last seen 2026-07-14, yellow)
- ⬜ [dh:backlog-total] The unpromoted backlog grew from 67868 to 77212 rows since the last run; capture is outpacing promotion. Action: Schedule a promotion session (promote-safe --create-new) or grow the dictionary coverage. (first seen 2026-07-12, last seen 2026-07-14, yellow)
- ⬜ [dh:contamination-trr] 7 TRR rows captured in the last week would fail today's non-bag filter, so non-bag items are leaking back in. Action: Run the TRR non-bag cleanup scripts (cleanup-trr-nonbag.ts) and check the ingest gate. (first seen 2026-07-10, last seen 2026-07-14, yellow)
- ⬜ [dh:contamination-misattach] 47 accessories (card holders, pouches, shoes) landed on a real bag's variant in the last week, which is exactly what put wrong items on the deals rail; 174 more look like a different-model bag and want a manual review. Action: Run supabase/ingest/detect-listing-discrepancies.ts (dry run to review, --write to re-point the accessories, --tag to label), and check why the ingest match let them onto a bag variant. (first seen 2026-07-12, last seen 2026-07-14, red)
- ⬜ [dh:contamination-size] 817 listings captured in the last week state a size that differs from the variant they landed on (e.g. a mini on the Medium page), which unpopulates the size selector and skews same-size pricing. Action: Run supabase/ingest/resize-variants.ts (dry run, then --write) to re-point them, and check the load-prices size guard. (first seen 2026-07-12, last seen 2026-07-14, red)
- ⬜ [dh:contamination-dupes] 317 listings were written more than once for the same observation day, which contaminates medians until deduped. Action: Inspect the newest loads; the loader should upsert on platform|listing_ref|price_type|observed_on. (first seen 2026-07-10, last seen 2026-07-14, red)
<!-- data-health:end -->
