...
**Status:** Seeded as a DRAFT (slug `how-to-authenticate-a-coach-bag`, topic Coach). NOT published. Awaiting owner review, then owner publishes.
**Lane:** Content. **Topic tag (for the CTA block):** Coach (brand).
**Bound by:** `docs/authentication-standard.md` (§7 gate, audited below) + `docs/voice-and-tone.md` + the factuality protocol in `docs/preferences.md`.
**Visual:** the `[diagram: coach-authentication]` token renders `src/app/posts/[slug]/CoachAuthDiagram.tsx` (full-width tiles, each with a "Read more" anchor to its body section). Diagram = fast scan; body = the deeper reference, deliberately different language so it never repeats the diagram.

---

## §7 pre-publish gate — self-audit

- [x] Every marker is objective, cross-verified (2+ reputable sources), era-stamped, and failure-mode labeled (§1). Source map below.
- [x] Confidence governs publication + wording; no visible reliability badge (§1 #5, owner preference).
- [x] No verdict of authentic / genuine / real anywhere. Capped: a wrong marker is a red flag, a right marker is never proof.
- [x] The "we do not guarantee authenticity" line + escalate-to-a-human route present (§4), in a callout.
- [x] Diagram is original schematic line art; no licensed/eBay/traced photo; no logo/trademark canvas reproduced (§3, §6); good/bad pairs carry check + X, not color alone.
- [x] Coach cleared the §5 validatable bar; escalation line present.
- [x] Voice: no em dashes; jargon (creed, YKK, signature canvas, glove-tanned) defined in plain words; body does NOT duplicate the diagram's phrasing.

## Sources (reputable, cross-verifying)
- **Fashionphile, "How We Look at Authentication"** — https://www.fashionphile.com/pages/authenticity
- **Thanks It's Vintage, "How to Spot a Fake Coach Bag"** — https://www.thanksitsvintage.co.uk/en-us/blogs/news/how-to-spot-a-fake-coach-bag
- **PurseForum Coach + leather threads** — https://forum.purseblog.com/threads/authenticate-this-coach-see-first-post-for-format.889527/ and https://forum.purseblog.com/threads/answers-to-authenticity-questions.94198/
- Supporting only: Luxtodo, Your Next Shoes.

## Per-marker source map
1. Creed (heat-stamped, sewn in, placement varies): Fashionphile + Thanks It's Vintage + PurseForum.
2. Serial (embossed, >5 digits, not N-Z, 2014-16 white tag, asymmetry): PurseForum + Thanks It's Vintage + Fashionphile.
3. Stitching: Fashionphile + Thanks It's Vintage.
4. Signature canvas (coated fabric; front balanced; seams need not align; exceptions): Fashionphile + PurseForum.
5. Leather feel (glove-tanned, natural grain/pores, aniline-dyed, smell, patina; newer coated/thinner hedge): Thanks It's Vintage + PurseForum + Fashionphile.
6. Zipper myth: PurseForum.
...
> We do not authenticate bags and we do not guarantee authenticity. When real money is on the line, before you buy something expensive, or sell or insure it, put the bag in the hands of a professional authenticator who can examine it in person. That is the only step that confirms it.
