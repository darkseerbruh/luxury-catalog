# LV Alma value article — DRAFT (2026-07-10)

*Backlog item: "LV Alma: what it is + what it costs on resale" (search "alma" x3, category "What it's worth", Sofia, priority 1). Data bar verified from our catalog 2026-07-10, deduped by listing_ref. Copywriter-drafted (voice canon applied), reviewed for factuality. NOT published. Renderer-token compatible (##, -, >, **bold**, paragraphs; no tables/rules).*

**Owner review flags:**
- The **"Mini" size** (n=130) is a market/label bucket, not an official LV Alma size (LV uses Nano / BB / PM / MM / GM). Kept as-is because the data is real, but softened the characterization. Say the word if you want it folded into BB/Nano or dropped.
- **Year 1934** is the article-#34-verified heritage anchor (design origin). Applied to the Alma page's `year_introduced` too.
- No dedicated "Alma" entry in the demand map; title/H2s written to the "alma" + "what it's worth" search intent.
...
The Louis Vuitton Alma is the structured, dome-shaped one. Rounded silhouette, a top handle, a zip that runs the curve of the top. If you have seen a Louis Vuitton bag that looks a little like a small doctor's bag standing upright, that is usually the Alma.
