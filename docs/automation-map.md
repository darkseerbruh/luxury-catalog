...
| **Data health** (daily scorecard: freshness, capture sanity, ranking floor, coverage deltas, backlog, contamination, cadence audit; graduates to weekly after 7 greens, red demotes back) | DB `price_history` / `discovered_listing` / RPCs | thresholds in `src/lib/data-health-core.ts` | `data-health.yml` (daily 10:47 UTC): `npm run health:check -- --write` | `reports/data-health/` + `data-content-worklist.md` findings section + `data-health` GitHub issues on red | Cron/cadence changes + all non-trivial fixes (only auto-fix: summary matview refresh) |
