...
| **Data health** (daily scorecard: freshness, capture sanity, ranking floor, coverage deltas, backlog, contamination, catalog structure — pseudo-styles/title-junk/accent-dups + variant attr coverage, `src/lib/catalog-structure.ts` — cadence audit; graduates to weekly after 7 greens, red demotes back) | DB `price_history` / `discovered_listing` / `style`+`variant` / RPCs | thresholds in `src/lib/data-health-core.ts`; structure baseline in `docs/catalog-structure-audit-0714.md` | `data-health.yml` (daily 10:47 UTC): `npm run health:check -- --write` | `reports/data-health/` + `data-content-worklist.md` findings section + `data-health` GitHub issues on red | Cron/cadence changes + all non-trivial fixes (only auto-fix: summary matview refresh) |
...
| **Vendor inbox** (tool/vendor mail → fix) | ariellecoambes@gmail.com (Gmail connector; arielle@luxurycatalog.com forwards here) | `vendor-inbox-engine.md` | `vendor-inbox-scan` (twice daily) | `vendor-inbox-ledger.md` | Merge the PR; spend; rotate keys; reply |
