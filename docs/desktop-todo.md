...
- [x] 📣 **D3. DONE 2026-07-06 (checked in GSC via Chrome): 4 redirect URLs = apex http/https + www-http variants of the homepage (expected canonicalization) + `/watchlist` (signed-in page, redirect is correct, verified NOT in the sitemap). Nothing to fix.** Original item: Diagnosed + code-fixed 2026-07-03: `/deals` was listed in the sitemap but always redirects to `/shop` (removed from sitemap); the rest is expected canonicalization (old `/posts/*`+`/closets` URLs 308 to their new homes; apex + vercel.app hosts 308 to www) — those SHOULD stay redirects, do not "fix" them. **Your check:** GSC → Indexing → Pages → "Page with redirect" → confirm the listed URLs are only `/deals`, `/posts/*`, `/closets`, apex/http variants. Anything else (random product pages, redirect loops) → flag it in a chat.
- [x] 📣 **D4. DONE 2026-07-06 (checked in GSC via Chrome): the offers/review/aggregateRating issue shows Validation PASSED, Invalid 0 / Valid 1 (GSC last update 7/4/26) — Google re-crawled and validated the 07-03 fix on its own, no Validate Fix click needed. Bag pages are rich-results eligible.** Original item: Root cause found + code-fixed 2026-07-03: our Product JSON-LD had no `offers`/`review`/`aggregateRating` (Google treats that as critical → bag pages ineligible for product rich results). Bag pages now emit an `AggregateOffer` built from the page's own current asking listings (low/high/count/currency; omitted when a variant has no listed rows — never invented) + the variant image. **Your check (2 min, after Vercel deploys `main`):** GSC → Shopping (or Enhancements) → Product snippets → the "Either offers, review, or aggregateRating…" issue → **Validate Fix**. Optional sanity: paste a bag URL with listings into the Rich Results Test and confirm the Product shows an offers block.
...
- [ ] 💤 **H7. Follow a house — PARKED as an aspirational future feature (owner call, 2026-06-27).** Removed from the UX because nothing feeds it yet: following a brand implies updates we can't deliver (no brand feed). **Revive only once the data-derived "market-move cards" feed exists** (price/median moves on followed houses + bags, built from data we already pull — NOT a manually-curated new-season release calendar, which is the upkeep trap the owner rejected). The built code (Follow control + `brand_follow` table + `0032`) was deleted this session but lives in git at commit `47618ee` (and merge `1fad6de`); reviving is a cherry-pick, so no working code was truly lost. **Strategy lens:** the feed half lands on the engagement→return-visit→affiliate arrow (already "BUILD, highest priority" in `engagement-strategy.md`); Follow without it is engagement theater. Build feed first, then re-add Follow as its front door.
