...
- [ ] **Connect Google Search Console** and submit `/sitemap.xml` (Bing Webmaster Tools too; Bing powers ChatGPT search). Status: OPEN (DNS already live).
...
- [x] **Analytics hygiene + premium fake-door** (this branch, `ops/monetization-hygiene-0706`): pulse journey now reads `article_viewed` / `bags_compared` / `attribute_object_viewed`; dead `style_viewed` removed; "Deal alerts Pro" coming-soon door on the watchlist + a quiet mention at the bag-page bell moment, firing `monetization_interest` with a `source` (event-only v1, PostHog is the interest list). Status: DONE, awaiting her merge.
- [ ] **Newsletter opt-in** (separate branch): the opt-in surface for the owned-audience channel; `newsletter_subscribed` already in the taxonomy. Status: NOT STARTED.
- [ ] **Market-report generator** (separate branch): the recurring data-report asset off the price DB. Status: NOT STARTED.
