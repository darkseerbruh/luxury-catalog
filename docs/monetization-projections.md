...
3. **Rental affiliate is the new 5th stream.** Vivrelle (luxury bag/jewelry rental membership, **20%/sale via Awin**; the Awin *network* account is held, but the Vivrelle *advertiser* program still needs per-advertiser approval and is **Pending as of 2026-06-27**) and Rent the Runway (**7% via Skimlinks**, already covered). It's a recurring-membership referral that maps onto the **`want`** intent ("not ready to buy? rent it first") — a clean third transaction type alongside buy/sell.
