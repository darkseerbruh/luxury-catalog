...
3. **Rental affiliate is the new 5th stream.** Vivrelle (luxury bag/jewelry rental membership, **20%/sale via Awin**; the Awin *network* account is held, but the Vivrelle *advertiser* program still needs per-advertiser approval and is **Pending, nudged 2026-07-05; live status board in `docs/data-collection-handoff.md` §11**) and Rent the Runway (**7% via Skimlinks — now BLOCKED: Skimlinks declined the site 2026-06-25, 90-day lockout, re-source after ~2026-09-25**). It's a recurring-membership referral that maps onto the **`want`** intent ("not ready to buy? rent it first") — a clean third transaction type alongside buy/sell.
...
| **2. Rental affiliate** | ~M2 | conversions × **~$40 blended** (Vivrelle 20% via Awin dominates; RTR 7% via Skimlinks) | Recurring-membership referral on the `want` intent. Awin network account held, but the Vivrelle program is **Pending approval (as of 2026-06-27)**, so this stream stays gated until it clears. Modest but additive; partly offsets the consignor loss. |
