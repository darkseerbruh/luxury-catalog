...
- **TRR non-bag ingest filter** (2026-07-10) — a broad TRR brand search returns that house's apparel/shoes/jewelry/watches/accessories too; those can never promote to a bag style (the SLG gate refuses them) so they bloated `discovered_listing`. `isTrrHandbagListing(url, title)` in `src/lib/ingest/trr.ts` reads the department out of the TRR product-URL path (keep `/handbags/`+`/bags/`, drop clothing/shoes/jewelry/watches/accessories/home/kids; carried-pouch escape for WOC/vanity/belt bag). Wired into both catch-all adapters + the curated `trr-jsonld` mapper. One-time purge: `npx tsx supabase/ingest/cleanup-trr-nonbag.ts [--write]` (dry-run default, keyset paginated, unpromoted-only). First run removed **8,126** banked non-bags; 0 remain. ~155 non-bags that had *already* promoted onto bag variants are flagged, not touched (separate price_history cleanup — spun off as its own task).
...
3.5 **Condition recording + backfill (built 2026-06-29, branch `claude/condescending-payne-df67c3`).** Root cause of the empty `condition` column: TheRealReal (the dominant source) hardcoded `condition: null`. Fixed: `trr-jsonld.ts` + `firecrawl-trr.ts` now tag genuinely-new TRR listings `new` and stash raw `itemCondition` into `condition_detail`, so future captures record it. Shared mapper `src/lib/ingest/condition.ts` (`mapConditionText` / `mapTrrItemCondition`) never invents a grade (ambiguous → null). **OWNER-GATED run (needs prod `SUPABASE_SERVICE_ROLE_KEY`; TRR refetch also needs `FIRECRAWL_API_KEY`):** `npm run condition:coverage` (before) → `npm run backfill:condition -- --mode=map --write` (free; re-grades stored text) → `npm run backfill:condition -- --mode=refetch --source=realreal --max-credits=2000 --write` (Firecrawl, caps spend) → `condition:coverage` (after). Dry-run is the default (omit `--write`). Then make the rail verdict condition-aware (compare like-condition comps).
