...
const EMPTY: MarketPulse = { totalPrices: 0, bags: 0, houses: 0, earliestYear: null, byHouse: [] };
...
async function loadMarketPulse(): Promise<MarketPulse> {
  ...
  try {
    ...
    if (!summaryRes.error && summaryRes.data) {
      for (const s of summaryRes.data as { variant_id: number; resale_n: number | null }[]) {
