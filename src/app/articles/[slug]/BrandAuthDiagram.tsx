...
const BRANDS: Record<string, BrandAuth> = {
  ...
  "fendi-authentication": {
    house: "Fendi",
    subhead: "A wrong marker is a red flag. A right marker is never proof.",
    sources: "Sourced from authentication services and reseller guides (Fashionica, TheRealReal, 9ine Life), checked June 2026. The FF print and hologram are the most-copied elements, so the read comes from execution, not the logo.",
    markers: [
      { name: "Serial and tag, by era", where: "interior tab", kind: "info",
        body: "Pre-1980s pieces often have no serial, the 1980s to 2000s use a serial leather tag, a Fendi SAS marking points to that older era, the 2000s added hologram tags, and recent bags add a QR code or NFC chip. Match the format to the age.",
        note: "match the format to the claimed age" },
      { name: "FF logo geometry", where: "the Zucca print", kind: "good-bad",
        body: "The double-F is sharp-edged, symmetrical, and holds a set width-to-height ratio. On Zucca print the two F's sit slightly offset on either side of the rectangle rather than perfectly mirrored, and the print should never look stretched.",
        good: "sharp, offset F's", bad: "stretched or mirrored" },
      { name: "Zucca canvas tone", where: "the body", kind: "good-bad",
        body: "Genuine Zucca canvas has a tight weave, clean print registration, and a warm tone. It should never read green, and loose weave or color bleed is a flag.",
        good: "warm tone, tight weave", bad: "green, loose, bleeding" },
      { name: "Hardware and screws", where: "clasp backing", kind: "good-bad",
        body: "The FENDI engraving on hardware is crisp and properly deep, not shallow or surface-etched. On the Baguette clasp backing the screws are flat-head, so a Phillips or star screw is a tell.",
        good: "deep engraving, flat screws", bad: "shallow, Phillips screws" },
      { name: "Made in Italy stamp", where: "interior", kind: "flag",
        body: "The origin line is pressed into the leather in the correct font and reads Made in Italy only. Any other country is a definitive fake tell. Note that Fendi Roma is a brand mark, which is different from the origin line.",
        note: "any non-Italy origin is a definitive tell" },
      { name: "Lining, sewn not glued", where: "inside", kind: "good-bad",
        body: "The lining is neatly sewn in with no bunching. Glued lining or exposed raw edges are a flag.",
        good: "sewn, no bunching", bad: "glued, raw edges" },
    ],
  },
  "loewe-authentication": {
    house: "Loewe",
    subhead: "Mostly craft judgment, not a code. A wrong marker is a flag, a right one is never proof.",
    sources: "Sourced from authentication services and reseller guides (Fashionica, TheRealReal, Legitique), checked June 2026. There is no public Loewe serial lookup and no authenticity card, so much of this is craft judgment, flagged where it is.",
    markers: [
      { name: "The Anagram symmetry", where: "the four-L logo", kind: "good-bad",
        body: "The four interlocking L's must be identical in weight, size, and spacing, and centered. Asymmetry, thick-to-thin variation, or an off-center mark is the most common tell.",
        good: "identical, centered", bad: "asymmetric, off-center" },
      { name: "The O in LOEWE", where: "the wordmark", kind: "good-bad", note: "lower confidence, verify on close photos",
        body: "On the wordmark the O is not a perfect circle, it has slightly pointed ends, a subtle football shape. A perfectly round O is a known tell, though this one is lower confidence, so check it on close photos.",
        good: "subtly pointed O", bad: "perfectly round O" },
      { name: "Leather hand", where: "the body", kind: "good-bad", note: "craft judgment, not a hard tell",
        body: "Authentic Nappa calfskin is exceptionally soft, supple, fine-grained, and slightly heavy. Papery, stiff, or plasticky leather, or an oversized uniform grain, points the other way. This is feel-based, so weigh it as craft judgment.",
        good: "soft, fine grain", bad: "stiff, large grain" },
      { name: "Puzzle panels and seams", where: "the geometric panels", kind: "good-bad",
        body: "On the Puzzle the panels fit with precision, no gaps or misaligned edges, and the seam lines between them are thick, bold, and clearly defined. Thin seams that fade into the leather are a tell.",
        good: "precise panels, bold seams", bad: "gaps, thin seams" },
      { name: "Hardware engraving", where: "metal Anagram", kind: "good-bad",
        body: "On hardware the Anagram is deeply, precisely engraved and fully fills the metal square. A logo floating with big margins is a flag, and the zips are Lampo or Riri.",
        good: "fills the square", bad: "floats with margins" },
      { name: "Tag and serial", where: "interior leather tab", kind: "good-bad",
        body: "The tab reads LOEWE over Made in Spain over the serial, with defined corners, and the serial is debossed into the leather rather than printed. Mainline handbags are Made in Spain, so a non-Spain origin on a current bag is a strong flag.",
        good: "debossed serial, Made in Spain", bad: "printed serial, off origin" },
    ],
  },
};
