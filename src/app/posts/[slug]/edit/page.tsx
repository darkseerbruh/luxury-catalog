...
export default async function EditPostPage({
  ...
  if (!profile?.isExpert) redirect("/posts");

  const post = await getBySlug(slug);
  if (!post) notFound();
  // Only the author may edit (RLS also enforces this; this is the UI guard).
  if (post.author?.userId !== user.id) redirect(`/posts/${post.slug}`);
    ...
    <main className="mx-auto flex w-full max-w-2xl flex-col gap-8 px-5 py-10">
      <header>
        <p className="text-sm uppercase tracking-widest text-muted">
          <Link href="/profile/posts" className="hover:text-gold">My articles</Link> / Edit
        </p>
        ...
        <p className="mt-2 text-muted">
          ...
          <Link href={`/posts/${post.slug}`} className="text-gold hover:underline">View live →</Link>
        </p>
      </header>
      <PostForm
        brands={brands}
