...
const FITS_LABELS: Record<string, string> = {
  ...
  phone: "Fits a cell phone",
  tablet: "Fits a tablet or book",
  laptop: "Fits a laptop",
};

export default async function BrowseFitsPage({
  params,
}: {
  params: Promise<{ item: string }>;
}) {
  const { item } = await params;
  const label = FITS_LABELS[item] ?? `Fits a ${item.replace(/-/g, " ")}`;
  const variants = await getVariantsByFits(item);
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-5 py-10">
      ...
      <nav className="flex items-center gap-1.5 text-sm text-muted">
        ...
        <span>/</span>
        <Link href="/#fits" className="hover:text-foreground">
          By what they fit
        </Link>
        <span>/</span>
      ...
      <header>
        <h1 className="font-serif text-4xl text-foreground">{label}</h1>
