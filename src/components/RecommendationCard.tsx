...
import { QuickSaveHeart } from "./QuickSaveHeart";
...
export function RecommendationCard({
  ...
  return (
    <div className="relative flex min-w-[200px] max-w-[220px] flex-shrink-0 sm:min-w-0">
      <QuickSaveHeart variantId={rec.variantId} source={source} className="absolute right-2 top-2 z-10" />
      <Link
        href={`/bag/${rec.variantId}`}
        onClick={() => track(EVENTS.recommendationClicked, { variant_id: rec.variantId, source })}
        className="flex w-full flex-col rounded-2xl border border-border bg-surface p-4 transition-colors hover:border-gold"
      >
