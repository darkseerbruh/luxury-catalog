...
async function main() {
  ...
  type P = { brand: string; brandId: number; style: string; size: string; count: number; cluster: DiscoveredCluster };
  const plan: P[] = [];
  ...
  for (const p of plan) {
    ...
    let variantId = (existingVars ?? []).find((v: VariantRow) => sizeKey(v.size_label) === sizeKey(p.size))?.variant_id;
    if (!variantId) {
