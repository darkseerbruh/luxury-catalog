/**
 * Un-promote + de-contaminate TRR NON-bag listings that ALREADY reached the catalog.
 *
 * Sibling to cleanup-trr-nonbag.ts, which purges *unpromoted* non-bag rows from
 * discovered_listing and deliberately LEAVES the ones that already promoted ("non-bag
 * rows already promoted — left in place, review manually"). This pass is that review:
 * a "Triomphe" belt / hoodie / wallet, a "Sylvie" shoe, an "Alma" bag-strap etc. shares
 * a brand+model token with a real bag, so the clustered promoter (promote-safe.ts) put it
 * on the bag's variant and wrote its (much cheaper) asking price into price_history,
 * deflating that bag's comps. See [[trr_mismap_sweep_method]] for the title-based sibling.
 *
 * Non-bag is judged by isTrrHandbagListing(source_url, title) — TRR's URL department path,
 * title-aware so a carried pouch the catalog ranks (WOC / vanity / belt bag) that lives
 * under /accessories/ is KEPT, not dropped. This pass:
 *   1. finds every TRR price_history row that is a non-bag  → DELETE (preserving any row
 *      with no discovered_listing backup first, so nothing is lost),
 *   2. finds every promoted TRR discovered_listing row that is a non-bag → UN-PROMOTE
 *      (promoted_variant_id/promoted_at → null, unresolved_reason 'non_bag') so the
 *      sibling can purge it and it can never re-seed a bag's comps,
 ...
 * a deleted price_history row is always preserved in discovered_listing as raw evidence
 * first — the un-promoted rows ARE that archive; the few price rows that reached the
 * catalog via direct load-matching (no discovered backup) are banked before deletion.
 *
 *   npx tsx supabase/ingest/cleanup-trr-promoted-nonbag.ts            # DRY RUN (report only)
 *   npx tsx supabase/ingest/cleanup-trr-promoted-nonbag.ts --write    # apply
 */
...
async function main() {
  console.log(`cleanup-trr-promoted-nonbag: ${WRITE ? "WRITE" : "DRY RUN"}\n`);

  // 1) price_history contamination — authoritative scan by URL department path (title-aware).
  ...
  const phNonBag = ph.filter((r) => !isTrrHandbagListing(r.source_url, r.notes));
  ...
  const promoted = await pageAll<DiscRow>(
    "discovered_listing",
    "discovered_id, source_url, raw_name, promoted_variant_id",
    (q) => q.eq("platform", PLATFORM).not("promoted_variant_id", "is", null),
  );
  const nonBagPromoted = promoted.filter((r) => !isTrrHandbagListing(r.source_url, r.raw_name));
  ...
  // Contaminated price rows with NO discovered_listing backup → must preserve before delete.
  ...
  // (c) Un-promote the non-bag discovered rows so the sibling can purge them.
  ...
  for (const v of affectedVariants) {
    ...
    console.log(`   v${v} ${vName.get(v) ?? "?"}\n       before: ${fmt(before.get(v))}\n       after:  ${fmt(after.get(v))}`);
  }
  console.log("\n✅ done.");
}

main().then(() => process.exit(0)).catch((e) => {
  console.error("cleanup-trr-promoted-nonbag failed:", e instanceof Error ? e.message : e);
  process.exit(1);
});
