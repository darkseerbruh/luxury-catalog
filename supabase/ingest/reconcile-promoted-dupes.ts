...
type CreatedVariant = { variant_id: number; style_id: number; size_label: string | null; created_at: string };
type SibVariant = { variant_id: number; size_label: string | null; created_at: string };
type PhKeyRow = { platform: string | null; listing_ref: string | null; price_type: string | null; observed_on: string | null };

async function main() {
  // variants created today (the promote-safe batch)
  const created: CreatedVariant[] = []; let from = 0;
  for (;;) {
  ...
  for (const v of created) {
    ...
    const matches = ((sibs as SibVariant[] | null)||[]).filter((s)=> s.variant_id!==v.variant_id && tokenOf(s.size_label)===tokenOf(v.size_label) && tokenOf(v.size_label)!==null && new Date(s.created_at) < new Date("2026-06-26"));
  ...
  let movedRows = 0, deletedRows = 0, deletedVariants = 0;
  for (const m of merges) {
    ...
    const origKeys = new Set(((origRows as PhKeyRow[] | null)||[]).map((r)=>`${r.platform}|${r.listing_ref}|${r.price_type}|${r.observed_on}`));
    ...
    const { error: dErr } = await db.from("discovered_listing").update({ promoted_variant_id: m.orig }).eq("promoted_variant_id", m.dup);
    ...
    movedRows += toMove.length; deletedRows += toDelete.length;
