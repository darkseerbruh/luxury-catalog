/**
 * Redeluxe MASTER crawler (SERVER-SIDE, no browser) — captures EVERY active
 * listing from Redeluxe, our flagship reseller + creator partner (Georgia Swain).
 *
 * Redeluxe runs on Shopify; `/products.json` answers plain Node fetches (verified
 * 2026-07-10, HTTP 200). It is bag-focused, so the root feed is the inventory; the
 * adapter (redeluxe.ts) filters to handbags and resolves each via canonicalModel.
 *
 *   npx tsx supabase/ingest/sources/redeluxe-crawl.ts [--max-pages=N]
 *     --max-pages=N   safety cap (default 200 → up to 50k listings)
 *
 * Merges (dedup by url) into the raw dump `data/ingest/_raw/redeluxe.json` that
 * `redeluxe.ts --raw` consumes, and overwrites the live snapshot the reconcile step
 * diffs against. Mirrors fashionphile-crawl.ts (same pacing/backoff posture).
 *
 * Legal posture: prices are facts; every row carries source_url. Never ingest photos
 * or verbatim descriptions (the adapter mines stated facts only). Rate-limit politely.
 */
import fs from "fs";
...
const LIVE_SNAPSHOT = path.resolve(__dirname, "../../../data/ingest/_raw/redeluxe-live.json");
const UA =
...
async function fetchPage(page: number): Promise<RawDumpEntry["product"][] | null> {
  const url = `https://redeluxe.com/products.json?limit=${LIMIT}&page=${page}`;
...
async function main() {
  ...
  console.log(`Redeluxe master crawl: /products.json (pages ${startPage}–${maxPages})`);
  ...
  for (let page = startPage; page <= maxPages; page++) {
    ...
    for (const p of products) {
      fresh.push({
        ...
        url: `https://redeluxe.com/products/${p.handle}`,
      });
    }
  ...
  console.log(`Crawled ${fresh.length} listing(s) from Redeluxe.`);
  ...
  // Current-run LIVE snapshot (overwrite, NOT merged): exactly what this crawl saw right
  // now. reconcile-sold.ts diffs against it to retire listings that have since sold/pulled.
  // Only written for a full crawl (start at page 1) — a resumed/partial crawl must not be
  // mistaken for the whole live inventory.
  if (startPage === 1) {
