/**
 * Ann's Fabulous Finds MASTER crawler (SERVER-SIDE, no browser) — an open-Shopify
 * consignor (trusted since 2008), real houses, clean bag titles.
 *
 * Runs on Shopify; `/products.json` answers plain Node fetches (verified 2026-07-10,
 * HTTP 200 on the bare host). It sells shoes/jewellery too, so the adapter (anns.ts)
 * filters to bag product_types and resolves each via canonicalModel.
 *
 *   npx tsx supabase/ingest/sources/anns-crawl.ts [--max-pages=N]
 *     --max-pages=N   safety cap (default 200 → up to 50k listings)
 *
 * Merges (dedup by url) into `data/ingest/_raw/anns.json` that `anns.ts --raw` consumes, and
 * overwrites the live snapshot the reconcile step diffs against. Mirrors redeluxe-crawl.ts.
 *
...
const LIVE_SNAPSHOT = path.resolve(__dirname, "../../../data/ingest/_raw/anns-live.json");
const UA =
...
async function fetchPage(page: number): Promise<RawDumpEntry["product"][] | null> {
  const url = `https://annsfabulousfinds.com/products.json?limit=${LIMIT}&page=${page}`;
...
async function main() {
  ...
  console.log(`Ann's Fabulous Finds master crawl: /products.json (pages ${startPage}–${maxPages})`);
  ...
  for (let page = startPage; page <= maxPages; page++) {
    ...
    for (const p of products) {
      fresh.push({
        ...
        url: `https://annsfabulousfinds.com/products/${p.handle}`,
      });
    }
  ...
  console.log(`Crawled ${fresh.length} listing(s) from Ann's Fabulous Finds.`);
